...
"""Manage Hopper's OpenRouter model list for a catalog installation."""
...
def _initialize_defaults(path: Path) -> None:
    """Create a first-run catalog so the provider has useful models immediately."""
    if path.exists():
        return
...
def read_models() -> list[str]:
    path = models_file()
    # Catalog installs must expose Hopper's defaults before any fetch_models()
    # call.  Initialization is deliberately explicit and confined to this
    # helper; an existing empty file remains a valid user choice.
    _initialize_defaults(path)
    if not path.exists():
...
def cmd_dump(_: argparse.Namespace) -> int:
    """Print only model IDs for Hopper's Desktop page."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Hopper models in Hermes.")
    ...
    p = sub.add_parser("file", help="Print the models.txt path")
